...
let pool;
try {
  pool = require('../db'); // common pattern: module.exports = pool
...
router.get('/', async (req, res) => {
  try {
    ...
    const params = [];
    let i = 1;
    if (course_id) { q +=  AND course_id = -Raw{i++}; params.push(course_id); }
    if (lecturer_id) { q +=  AND lecturer_id = -Raw{i++}; params.push(lecturer_id); }
    if (date_from) { q +=  AND start_time >= -Raw{i++}; params.push(date_from); }
    if (date_to) { q +=  AND end_time <= -Raw{i++}; params.push(date_to); }
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!pool || !pool.query) return res.status(500).json({ error: 'database pool not available' });
    const q = 'SELECT id, title, course_id, lecturer_id, start_time, end_time, capacity, created_at FROM sessions WHERE id =  LIMIT 1';
